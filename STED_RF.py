...
def rf_eval(max_depth, min_samples_split, min_samples_leaf):
    params = {'n_estimators':100,'max_depth':int(round(max_depth)), 'min_samples_split':int(round(min_samples_split)), 'min_samples_leaf':int(round(min_samples_leaf))}
...
pbounds = {'max_depth':(5, 20), 'min_samples_split':(2, 10), 'min_samples_leaf':(1, 10)}
rf_bayesopt = BayesianOptimization(f=rf_eval, pbounds=pbounds, random_state=42)
rf_bayesopt.maximize(init_point=5, n_iter=5)
